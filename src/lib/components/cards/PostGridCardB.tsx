	...
	return (
		<div className="neu-base group relative flex h-[320px] max-h-[350px] max-w-[480px] flex-col overflow-hidden rounded-md">
			<div className="absolute z-[1] flex h-[80px] w-full items-center justify-center bg-background px-12 py-2 text-center transition-[background-color_height] group-hover:h-full group-hover:bg-background/60 group-hover:px-4">
				<Link href={url}>
					<h4 className="truncate-2 rounded box-decoration-clone font-bold ml:text-[22px] text-[18px] group-hover:line-clamp-3 group-hover:inline group-hover:text-ellipsis group-hover:bg-background group-hover:px-2 group-hover:text-3xl">
						{title}
					</h4>
				</Link>
				{session != null && (
					<span className="absolute top-6 right-4 text-3xl">
						<SavedIcon />
					</span>
				)}
			</div>
