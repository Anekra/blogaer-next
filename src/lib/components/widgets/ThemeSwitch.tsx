...
export default function ThemeSwitch({
	...
	return (
		<div
			className={`${isHover ? "neu-base bg-accent" : "bg-background"} ${width} switch-container h-full`}
		>
			<input
				id="autosave-switch"
				type="checkbox"
				className="absolute z-1 size-full cursor-pointer appearance-none rounded-full"
				checked={isOn}
				onChange={(e) => {
			...
			/>
			<span
				className={`${isOn ? `${transform}` : "translate-x-0"} ${isHover ? "neu-base text-accent" : "text-base-background"} ${padding} switch-toggle`}
			>
				{isOn ? (
					<svg
