...
export async function GET(request: NextRequest) {
	...
	if (!code && state) {
		...
		if (redirectUrl) {
			githubRedirectRes.cookies.set("redirect_url", `${redirectUrl}`, {
				httpOnly: true,
				sameSite: "strict",
				secure: true,
				maxAge: 1 * 3 * 60
	...
	try {
		...
		const redirectCookie = request.cookies.get("redirect_url")?.value;
		if (!res.ok) {
			if (redirectCookie) errRedirect.cookies.delete("redirect_url");
			const searchParams = [
				{ param: "redirect", value: "Oauth login failed." },
				{ param: "error", value: resJson.message }
			];
			searchParams.forEach(({ param, value }) => {
		...
		nextRes.cookies.delete("redirect_url");

		await setSessionCookie(resJson, true);
